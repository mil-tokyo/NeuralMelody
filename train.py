...
from reg_range import get_reg_range_weights, get_reg_range_cost, get_reg_range_derivative
...
def RNNGenCost(batch, model, params, misc):
  ...
  reg_range_weights = misc['reg_range_weights']
  reg_range_coeff = params['reg_range_coeff']
  ...
  for i,pair in enumerate(batch):
    img = pair['image']
    # ground truth indices for this sentence we expect to see
    ...
    ###########################################################################################
    # compute range regularization
    reg_range_cost_ = get_reg_range_cost(P, reg_range_weights)
    ...
    P += reg_range_coeff * reg_range_derivative   # Add range regularizarion cost
    dYs.append(P)
  ...
  out = {}
  out['cost'] = {'reg_range_cost' : reg_range_cost, 'reg_cost' : reg_cost, 'loss_cost' : loss_cost, 'total_cost' : loss_cost + reg_cost + reg_range_cost}
  out['grad'] = grads
...
def main(params):
  ...
  misc['reg_range_weights'] = get_reg_range_weights(misc['ixtoword'], params['reg_range_min'], params['reg_range_max'])
  ...
  for it in xrange(max_iters):
    ...
    print '%d/%d batch done in %.3fs. at epoch %.2f. loss cost = %f, reg cost = %f, range cost = %f, ppl2 = %.2f (smooth %.2f)' \
          % (it, max_iters, dt, epoch, cost['loss_cost'], cost['reg_cost'], cost['reg_range_cost'], \
...
#      abort = True # set the abort flag, we'll break out
    ...
    if (((it+1) % eval_period_in_iters) == 0 and it < max_iters - 5) or is_last_iter:
      ...
      if val_ppl2 < write_checkpoint_ppl_threshold or write_checkpoint_ppl_threshold < 0:
          ...
          filename = 'model_checkpoint_%s_%s_%s_%.2f_%.2f.p' % (dataset, host, params['fappend'], val_ppl2, epoch)
...
if __name__ == "__main__":
  ...
  # Sequence length regularization
  parser.add_argument('--reg_range_coeff', dest='reg_range_coeff', type=float, default=1, help='range regularization strength. Just set it to 0 to discard regularization')
  parser.add_argument('--reg_range_min', dest='reg_range_min', type=float, default=60, help='lowest pitch allowed')
  parser.add_argument('--reg_range_max', dest='reg_range_max', type=float, default=72, help='highest pitch allowed')
  ...
  parser.add_argument('-p', '--eval_period', dest='eval_period', type=float, default=5, help='in units of epochs, how often do we evaluate on val set?')
