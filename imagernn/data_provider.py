...
class BasicDataProvider:
  def __init__(self, dataset):
    ...
    dataset_path = os.path.join(self.dataset_root, 'data.json')
    print 'BasicDataProvider: reading %s' % (dataset_path, )
    self.dataset = json.load(open(dataset_path, 'r'))

    # load the image features into memory
    features_path = os.path.join(self.dataset_root, 'data.mat')
...
def getDataProvider(dataset):
  ...
  assert dataset in ['flickr8k', 'flickr30k', 'coco','music'], 'dataset %s unknown' % (dataset, )
